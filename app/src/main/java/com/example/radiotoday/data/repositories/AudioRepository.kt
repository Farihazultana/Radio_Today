...
class AudioRepository @Inject constructor(private val apiServices: ApiServices){
    ...
    suspend fun getAudioPlaylistData(page : String) : ResultType<SeeAllResponse> {

        try {
            val response = apiServices.postAudioPlaylistData(page)
            Log.i("TAGY", "successful api call: ${response.code()}")
            if (response.isSuccessful) {
                val data = response.body()
                if (data != null) {
                    Log.i("TAGY", "getAudioPlaylistData: ${data.content}")
                }
                if (data != null) {
