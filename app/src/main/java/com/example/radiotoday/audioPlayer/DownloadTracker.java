...
public class DownloadTracker {
    ...
    public void toggleDownload(
        ...
        if (download != null && download.state != Download.STATE_FAILED) {
            DownloadService.sendRemoveDownload(
                    context, FileDownloadExoService.class, download.request.id, false);
        } else {
    ...
    public void deleteDownload(MediaItem mediaItem) {
        @Nullable Download download = downloads.get(checkNotNull(mediaItem.localConfiguration).uri);
        if (download != null) {
            DownloadService.sendRemoveDownload(context, FileDownloadExoService.class, download.request.id, false);
        }
    }

    public MediaItem getMenuItem() {
        return mediaItem;
    }
    ...
    private final class StartDownloadDialogHelper
        ...
        public void onPrepared(DownloadHelper helper) {
            ...
            if (Util.SDK_INT < 18) {
                Toast.makeText(context, "Failed to obtain offline license", Toast.LENGTH_LONG)
                        .show();
                Log.e(TAG, "Downloading DRM protected content is not supported on API versions below 18");
                return;
            }
            // (internal b/163107948): Support cases where DrmInitData are not in the manifest.
        ...
        private void startDownload(DownloadRequest downloadRequest) {
            DownloadService.sendAddDownload(
                    context, FileDownloadExoService.class, downloadRequest, false);
        }

        private DownloadRequest buildDownloadRequest() {
            return downloadHelper
                    .getDownloadRequest(Util.getUtf8Bytes(Objects.requireNonNull(checkNotNull(mediaItem.mediaMetadata.title != null ? mediaItem.mediaMetadata.title.toString() : null))))
